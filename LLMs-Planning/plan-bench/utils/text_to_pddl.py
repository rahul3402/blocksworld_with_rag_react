...
def text_to_plan_blocksworld(text, action_set, plan_file, data, ground_flag=False):
    ...
    for k, v in data['actions'].items():
        ...
        if word in k:
            ...
            AD[k] = word
    #print("LD Mapping (Encoded Objects):", LD)
    #print("BD Mapping (Block Names):", BD)
    ...
    #print("Text Actions:", text_actions)    
    ...
    for line in lines:
        ...
        #print("Readable Objects:", readable_objs)
        #print("Mapped Objects:", objs)
        readable_action = "({} {})".format(action, " ".join(readable_objs[:n_objs + 1]))
        if not ground_flag:
            action = "({} {})".format(action, " ".join(objs[:n_objs + 1]))
        else:
