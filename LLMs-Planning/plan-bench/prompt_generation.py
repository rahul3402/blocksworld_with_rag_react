...
class PromptGenerator:
    ...
    def task_1_plan_generation(self, specified_instances=[], random_example=False):
        ...
        for start in tqdm(range_list):
            ...
            for i in range(start, start + self.n_examples + 1):
                ...
                print("FIRST ONESHOT EX:\n" + gt_plan_text)
                query += fill_template(*instance_to_text(problem, get_plan, self.data))
                print("Prompt End:\n" + fill_template(*instance_to_text(problem, get_plan, self.data)))
                print("FIRST QUERY:\n" + query)
                print("END")
                # --------------------------------------------------------- #
    ...
    def explain_reasoning(self, action):
        if "unstack" in action:
            return "Unstacking is required to make the block below clear and accessible."
        elif "put-down" in action:
            return "The block is no longer needed in hand and can be put down to free the hand."
        elif "pick-up" in action:
            return "Picking up the block is necessary to stack it onto another block."
        elif "stack" in action:
            return "The block is stacked to achieve the goal configuration."
        else:
            return "Default reasoning for this action."


    def task_1_plan_generation_with_react(self, specified_instances=[], random_example=False):
        task_name = f"task_1_plan_generation_react"
        instance_structured_outputs = []
        structured_output = self.load_json(task_name)
        completed_instances = []
        ...
        for start in tqdm(range_list):
            ...
            for i in range(start, start + self.n_examples + 1):
                ...
                # Read instance and compute plan
                problem = self.get_problem(cur_instance, self.domain_pddl)
                gt_plan = self.compute_plan(self.domain_pddl, cur_instance)
                gt_plan_text = get_plan_as_text(self.data)

                # Generate ReAct prompt
                query += f"Initial State:\n{instance_to_text(problem, False, self.data)[0]}\n\n"
                query += "Reasoning, Actions, and Observations:\n"
                ...
                for step in gt_plan.splitlines():
                    reasoning = self.explain_reasoning(step)
                    query += f"Reasoning: {reasoning} [END REASONING]\n"
                    query += f"Action: {step} [END ACTION]\n"
                    action_parts = step[1:-1]
                    action_parts = action_parts.strip().split()
                    ...
                    if "cost" not in step:
                        ...
                        try:
                            ...
                            observation = f"Error applying action '{step}': {e}"
                    else:
                        observation = ""

                    query += f"Observation: {observation} [END OBSERVATION]\n\n"
            # output_file = f"query_instance_{start}.txt"
            ...
            query += "\nRULES: MAKE SURE to follow to this format:\nReasoning: (insert reasoning) [END REASONING]\nAction: (insert action) [END ACTION]\nOberservation: (insert observation) [END OBSERVATION]\n\n"
            query += "Here is the mapping from color to letters:\n" + str(encoded_objects)
            query += fill_template(*instance_to_text(problem, get_plan, self.data))
            instance_structured_output["example_instance_ids"] = examples
            instance_structured_output["query"] = query
            instance_structured_output["ground_truth_plan"] = gt_plan_text
    ...
    def task_1_plan_generation_with_rag_react(self, specified_instances=[], random_example=False):
        task_name = f"task_1_plan_generation_react"
        instance_structured_outputs = []
        structured_output = self.load_json(task_name)
        completed_instances = []
        ...
        for start in tqdm(range_list):
            ...
            if 'caesar' in self.data['domain_name']:
                query = caesar_encode(query)
                stop_statement = caesar_encode(stop_statement)
            query += "\nYou will also be given a 'Reference History' with some previous Reasoning, Action, Observation steps that might be helpful and the 'Current State' from which you are supposed to continue planning from.\nRULES: MAKE SURE to follow to this format:\nReasoning: (insert reasoning) [END REASONING]\nAction: (insert action) [END ACTION]\nOberservation: (insert observation) [END OBSERVATION]\n\n"
            instance_structured_output["example_instance_ids"] = examples
            instance_structured_output["query"] = query
            instance_structured_output["ground_truth_plan"] = gt_plan_text
            structured_output["instances"].append(instance_structured_output)
