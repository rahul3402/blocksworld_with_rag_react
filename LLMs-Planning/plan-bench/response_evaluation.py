...
def extract_pddl_steps(input_text, plan_file):
    pattern = r'Action:\s*\**\s*(?:\((.*?)\)|([a-zA-Z0-9\- ]+))\s*\[END ACTION\]'
    
    matches = re.findall(pattern, input_text)
    allowed_actions = {"pick-up", "put-down", "stack", "unstack"}
    pddl_steps = []
    for match in matches:
        if match[0]:  
            action = f"({match[0].strip()})"
        else:  
            action = f"({match[1].strip()})"
        action_parts = action[1:-1].split()
        if action_parts[0] in allowed_actions:
            #formatted_action = f"({action})"
            pddl_steps.append(action + "\n")

    result = ''.join(pddl_steps)
...
def extract_pddl_steps_letters(input_text, plan_file):
    encoded_objects = {
        'red': 'a',
        'blue': 'b',
        'orange': 'c',
    ...
    for match in matches:
        ...
        for color, letter in encoded_objects.items():
            action = re.sub(rf'\b{color}\b', letter, action)
    ...
    def evaluate_plan(self, task_name):
        ...
        for instance_dict in tqdm(structured_output["instances"]):
            if "llm_raw_response" in instance_dict:
                ...
                try:
                    #llm_plan, _ = text_to_plan(llm_response, problem.actions, self.llm_plan_file, self.data, True)
                    #llm_response = "**Action:** unstack yellow orange [END ACTION]Action: (put-down yellow) [END ACTION]**Action:** pick-up orange [END ACTION]Action: (stack orange red) [END ACTION]"
                    llm_plan = extract_pddl_steps(llm_response, self.llm_plan_file)
                    if instance_dict["instance_id"] == 134:
                        print(llm_plan)
